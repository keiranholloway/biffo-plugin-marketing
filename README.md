...
> new repository, rename `marketing` throughout to your plugin's name,
...
│   ├── plugin.py            # MarketingPlugin(BiffoPluginBase) — the reference implementation
...
API); this template's `marketing` is deliberately the smallest version of
the same shape: one table (`marketing_widgets`), four routes, one idempotent
...
- **If it doesn't** — like `marketing`, which is event-only — there is no
...
| Setting                              | Value                                                                                                                                                                       |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Required status checks               | `strict = true`; contexts: `Lint`, `Type Check`, `Test`, `Admin frontend (lint, types, test, build)`, `Validate biffo.plugin.json`, `Terraform (fmt)`, `Secret Scan`, `Dependency Audit` — the job **names** from this repo's own `ci.yml`. Derive them rather than typing them: `biffo check branch-protection --repo <org>/<repo> --fix`. A job that exists but is not in this list is advisory only, which is how `web-admin/` ran ungated (#97) |
